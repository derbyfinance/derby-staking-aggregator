...
import "../Interfaces/ExternalInterfaces/IBeefy.sol";
...
contract BeefyProvider is IProvider {
  ...
  function deposit(
    ...
    console.log("befopre check");
    require((balanceAfter - balanceBefore - _amount) == 0, "Error Deposit: under/overflow");
    console.log("after check");
    uint256 bTokenBefore = IERC20(_bToken).balanceOf(address(this));
    IBeefy(_bToken).deposit(_amount);
    uint256 bTokenAfter = IERC20(_bToken).balanceOf(address(this));
    console.log("after deposit");

    uint256 bTokensReceived = bTokenAfter - bTokenBefore;
    console.log("bTokensReceived %s", bTokensReceived);
    IBeefy(_bToken).transfer(msg.sender, bTokensReceived);

    return bTokensReceived;
  }
  ...
  function exchangeRate(address _bToken) public view override returns (uint256) {
    uint256 price = IBeefy(_bToken).getPricePerFullShare();
    return price;
  }
