...
const deployFeeTestContract = (
  ...
  deployContract(deployerSign, FeeTestContractArtifact, [
    name,
    symbol,
    decimals,
    ETFname,
...
describe.skip('Testing feeTest, gas', async () => {
  ...
  beforeEach(async function () {
    ...
    // Get Addresses
    const [daoAddr, userAddr] = await Promise.all([dao.getAddress(), user.getAddress()]);
    const controller = await deployController(dao, daoAddr);
    feeTestContract = await deployFeeTestContract(
      dao,
      name,
      symbol,
      decimals,
