...
describe.skip('Testing VaultWithdrawOrder, integration test', async () => {
  ...
  it('Should deposit and withdraw in order from protocols', async function () {
    ...
    let totalUnderlying = (await vault.savedTotalUnderlying()).add(
      await IUSDc.balanceOf(vault.address),
    );
    ...
    // Total Compound
    let totalCompound = await compoundProvider.balanceUnderlying(
      vault.address,
      compoundVault.protocolToken,
    );
    ...
    expect(formatUSDC(totalUnderlying)).to.be.closeTo(100_000 - amountWithdrawed - gasUsed, 2);
    // Total liquid funds in vault == 0k
    totalLiquidFunds = await IUSDc.balanceOf(vault.address);
    expect(formatUSDC(totalLiquidFunds)).to.be.closeTo(0, 2);
