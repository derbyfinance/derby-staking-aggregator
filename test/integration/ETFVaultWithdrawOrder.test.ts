...
describe("Testing ETFVaultWithdrawOrder, integration test", async () => {
  ...
  it("Should deposit and withdraw in order from protocols", async function() {
    ...
    await vault.withdrawETF(userAddr, parseUSDC('20000'));
    ...
    await vault.withdrawETF(userAddr, parseUSDC('60000'));
    ...
    // Should be reverted
    await expect(vault.withdrawETF(userAddr, parseUSDC('60000'))).to.be.reverted;
  });
